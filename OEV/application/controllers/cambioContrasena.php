...
class CambioContrasena extends CI_Controller {
    ...
    //Vistas
		...
		$this->load->view('usuarios/administrador/menu_administrador');
			$usuarios = $this->usuariomodel->getUsuariosAA();
			$resultado = array('usuarios'=>$usuarios,"mensaje"=>"Contrase&ntilde;a cambiada!");
		if ($this->form_validation->run() == TRUE)
		{
            ...
            enviaMail($this,$this->usuariomodel->obtenEmail($this->input->post('susuario')),
					  ...
					  mensajeContrasena($this->usuariomodel->obtenNombre($this->input->post('susuario')),$this->input->post('password')));
			      $this->load->view('/usuarios/administrador/cambioContrasena',$resultado);
		}
		else
		{
