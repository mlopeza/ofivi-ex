...
class Logincontroller extends CI_Controller
{
    ...
    public function signup()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->departamento->find()) {
                $this->usuariomodel->insertarUsuario($this->departamento->get_id_departamento(), $this->input->post('username'), $this->input->post('nombre'), $this->input->post('apellido_paterno'), $this->input->post('apellido_materno'), $this->input->post('email'), $this->input->post('password'), $this->input->post('tipo-usuario'), 0, 'e');
    ...
    public function validaUsername()
    {
        ...
        if ($data['usuario'] == null) {
            ...
            $mensaje = array(
                'response' => 'false',
                'mensaje' => 'Error al recibir informacion.'
            );
    ...
    public function getEscuela()
    {
        ...
        if ($data['escuela'] == null) {
            ...
            $mensaje = array(
                'response' => 'false',
                'mensaje' => 'Error al Buscar los Proyectos.'
            );
            echo json_encode($mensaje);
        } else {
            //Regresa las empresas del Grupo
            $this->load->model('campus');
            $resultado = $this->campus->getEscuelas(array(
                "idCampus" => $data['escuela']
            ));
            //Regresa a categoria del proyecto.
            $this->load->model('escuela');
            $resultado2 = $this->escuela->getDepartamentos(array(
                "idEscuela" => $resultado[0]->idEscuela
            ));
