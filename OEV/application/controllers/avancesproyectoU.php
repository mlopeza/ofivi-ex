<?php
class AvancesproyectoU extends CI_Controller
{
    ...
    public function getEmpresas()
    {
        ...
        if ($data['idGrupo'] == null) {
            ...
            $mensaje    = array(
                'response' => 'true',
                'mensaje' => $resultado,
                'proyectos' => $resultado2,
                'categoria' => $resultado3,
                'usuario' => $resultado4,
                'contacto' => $resultado5,
                'estado' => $resultado6,
                'documento' => $resultado7,
                'supracategoria' => $resultado8
            );
            echo json_encode($mensaje);
        }
    }
    ...
    public function getGrupos()
    {
        ...
        if ($data['activo'] === null || !isset($data['activo'])) {
            ...
            $grupo = $this->grupo->getGPAU($data['activo'], $data['idUsuario']);
            //Regresa las empresas del Grupo
            if (!empty($grupo)) {
                $this->load->model('empresa');
                $resultado = $this->empresa->getEPAU($data['activo'], $grupo[0]->idGrupo, $data['idUsuario']);
                ...
                $mensaje = array(
                    'response' => 'false',
                    'mensaje' => 'No existen proyectos en la base de datos.'
                );
            }
            echo json_encode($mensaje);
        }
