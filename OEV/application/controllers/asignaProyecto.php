...
class asignaProyecto extends CI_Controller
{
    public function index()
    {
        ...
        $this->load->view('usuarios/usuario_extension/asignaProyecto', array(
            'proyectos' => $proyectos,
            'areas' => $areas
        ));
    ...
    public function asignaProfesor()
    {
        ...
        try {
            ...
            $this->proyecto->asignaEstado($data['data']['idProyecto'], $data['idUsuarioExtension'], 'Asignado (En Proceso)');
            //Mensaje al Usuario de Asignacion
            if ($respuesta == 1) {
                $mensaje = $this->load->view('mensajes/mensajeAsignacion', array(
                    'r' => $result1,
                    'p' => $result2
                ), true);
    ...
    public function eliminaAsignacion()
    {
        ...
        if ($this->db->_error_message() == null) {
            ...
            echo json_encode(array(
                'response' => 'false',
                'mensaje' => "Hubo un error en el Sistema, favor de intentarlo mas tarde." . $this->db->_error_message()
            ));
        }
    }
