...
class EnviaRecordatorios extends CI_Controller
{
    public function index()
    {
        ...
        $this->db->where(array(
            'enviado' => 0,
            'StartTime < ' => 'DAT_SUB(now(),INTERVAL 1 HOUR)'
        ));
        ...
        foreach ($result as $usuario) {
            ...
            $this->db->join('jqcalendar c', 'c.idUSuario = u.idUsuario', 'inner');
            $this->db->where(array(
                'u.idUsuario' => $usuario->idUsuario,
                'c.StartTime <' => 'DAT_SUB(now(),INTERVAL 1 HOUR)'
            ));
            ...
            $result  = $query->result();
            $mensaje = $this->load->view('mensajes/mensajeRecordatorios', array(
                'r' => $result
            ), true);
            ...
            if ($sent) {
                ...
                $this->db->update('jqcalendar', array(
                    'enviado' => 1
                ));
                echo "Enviado";
            } else {
